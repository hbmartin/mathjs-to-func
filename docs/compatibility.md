...
|-----------------------|--------|-------|
| `add`, `subtract`, `multiply`, `divide`, `pow`, `mod` | Supported | Uses Python/NumPy arithmetic. Also accepted in `FunctionNode` form. math.js percentage expansion such as `a + b%` is supported when serialized as either math.js's expanded tree or a raw percentage right operand. |
| `unaryPlus`, `unaryMinus`, `percentage` | Supported | Unary percentage is decoded as `x / 100`. |
| `not`, `and`, `or`, `xor` | Supported | Scalar `and`/`or` are lazy; arrays vectorize. Also accepted in `FunctionNode` form. |
| `equal`, `unequal`, `larger`, `largerEq`, `smaller`, `smallerEq` | Supported | Numeric comparisons use math.js-style default tolerances: `abs(a-b) <= max(absTol, relTol * max(abs(a), abs(b)))` with `relTol=1e-12`, `absTol=1e-15`; override per evaluator with `EvalConfig` or `{"epsilon": ...}`. Use `comparison="numpy"` for NumPy `isclose` semantics or `comparison="strict"` for exact equality. Also accepted in `FunctionNode` form. |
...
| Postfix factorial, transpose | Not supported | These parser operators are not translated today. Function-style `factorial(n)` is supported. |
...
|----------|--------|-------|
| `abs`, `ceil`, `exp`, `floor`, `log`, `logb`, `log1p`, `log2`, `log10`, `round`, `sign`, `sqrt`, `cbrt` | Supported | Uses NumPy unary helpers. `logb(x, base)` is an alias for `log(x, base)`. |
...
| `format` | Supported subset | Supports `format(value)`, `format(value, precision)`, and common options objects such as `{notation: "fixed", precision: 2}` for scalar and array-style UI output. |
...
- Constant arithmetic and selected literal helper calls are folded while building the Python AST. Repeated eager subexpressions inside an expression are hoisted to generated `__mj_cse_*` locals; lazy branches (`and`, `or`, `nullish`, conditionals, and relational chains) are not speculatively hoisted.
- Logical `and`/`or` preserve scalar short-circuit behavior. When the left operand determines the scalar result, the helpers may probe the right operand to preserve array-shaped mixed scalar/array results; arithmetic errors from that dead scalar branch are swallowed to match the short-circuit result.
- Result scalar demotion is controlled with `EvalConfig(result_dtype="auto"|"numpy"|"python")`. The default `"auto"` keeps the historical helper behavior; `"numpy"` preserves NumPy scalar/0-d outputs from helpers; `"python"` demotes final scalar outputs to Python scalars.
